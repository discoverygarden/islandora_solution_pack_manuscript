...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
   ...
   <xsl:template match="tei:pb">
      <!-- added by lisa -->
      <xsl:if test="count(preceding-sibling::tei:p[1]) = 0 and count(preceding-sibling::tei:milestone[1][@unit['page']]) = 0 "><br/></xsl:if>	 
      
	 <br/>
      <!-- end add by lisa -->
      <span class="norm header">
         <xsl:variable name="number" select="@n"></xsl:variable>
         <i>Page <xsl:value-of select="$number"/></i>
      </span>
      <br/>
   </xsl:template>
